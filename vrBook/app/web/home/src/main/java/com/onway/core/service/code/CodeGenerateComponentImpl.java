...
public class CodeGenerateComponentImpl implements CodeGenerateComponent {
	...
	protected static final Logger logger = Logger
			.getLogger(CodeGenerateComponentImpl.class);
	...
	public String nextCodeByType(final PlatformCodeEnum platformCode) {
		...
		if (sequenceNo <= 0) {
			throw new ErrorException(ResultCodeEnum.DATA_ERROR, "编码序列不存在:"
					+ platformCode.name());
		}
				...
				.execute(new TransactionCallback<Integer>() {
					...
					public Integer doInTransaction(TransactionStatus status) {
						...
						if (count == 0) { // 第一次,则数据初始化
							...
							sequenceDO
									.setCurrentValue(CodeGenerateConfig.INIT_VALUE);
							sequenceDO
									.setIncrementValue(CodeGenerateConfig.INCREMENT_STEP);
						...
						SequenceDO sequence = sequenceDAO
								.currentSequence(bizType);
	 ...
	 */
	public void setLocalNewTransTransactionTemplate(
			TransactionTemplate localNewTransTransactionTemplate) {
