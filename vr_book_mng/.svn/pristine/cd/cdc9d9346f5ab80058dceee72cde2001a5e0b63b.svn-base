...
public class RoleController extends AbstractController {
	...
	@ResponseBody
	public JSONObject selectrole(Integer offset,Integer limit,String selectname){
	...
	@ResponseBody
	public Object addrole(final SysRoleDO role ,final HttpServletRequest re){
		final JsonResult result = new JsonResult(true);
		controllerTemplate.execute(result, new ServiceCheckCallback() {
			
			@Override
			public void executeService() {
				role.setCreater(re.getSession().getAttribute("username")+"");
				role.setGmtCreate(new Date());
				role.setRoleId(codeGenerateComponent.nextCodeByType(PlatformCodeEnum.WEBSITE_PLATFORM));
	...
	@ResponseBody
	public Object updaterole(final String name,final HttpServletRequest re,final Integer id){
		final JsonResult result = new JsonResult(true);
		controllerTemplate.execute(result, new ServiceCheckCallback() {
			
			@Override
			public void executeService() {
				int updaterole = sysRoleDAO.updaterole(name, re.getSession().getAttribute("username")+"", new Date(), id);
	...
	public JSONObject selectjutisdiction(Integer roldid){
		JSONObject result = new JSONObject();
		JSONArray ja = JSONArray.parseArray(sysRoleDAO.selectjurisidiction(roldid));
	...
	@ResponseBody
	public Object updaterole(final int id,final String value, final HttpServletRequest re){
		final JsonResult result = new JsonResult(true);
		controllerTemplate.execute(result, new ServiceCheckCallback() {
			@Override
			public void executeService() {
				int updatejurisidiction = sysRoleDAO.updatejurisidiction(value, id);
	...
	public Object deleteRole(final int id ,final HttpServletRequest request){
		...
		controllerTemplate.execute(jsonResult, new ServiceCheckCallback() {
			
			@Override
			public void executeService() {
				int deleteRole = sysRoleDAO.deleteRole(id);
				jsonResult.setSuccess(deleteRole > 0 ? true:false);
				jsonResult.setMessage(deleteRole > 0 ? ErrorCode.SUCCESS.message():ErrorCode.SYSTEM_ERROR.message());
			}
